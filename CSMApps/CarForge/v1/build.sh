#!/bin/bash
# ============================================================
# build.sh — No-Gradle APK Build for CarForge v1
...
echo "============================================================"
echo "  CarForge v1 — No-Gradle aapt2 Build"
...
if [ -d "$SRC_DIR/assets" ]; then
    ASSET_TMP="$OBJ_DIR/atmp"; mkdir -p "$ASSET_TMP"
    cp "$OUT_DIR/$APP_NAME-base.apk" "$ASSET_TMP/base.apk"
    (cd "$SRC_DIR" && zip -r "$ASSET_TMP/base.apk" assets/ 2>/dev/null)
    mv "$ASSET_TMP/base.apk" "$OUT_DIR/$APP_NAME-base.apk"; rm -rf "$ASSET_TMP"
fi
...
(cd /tmp && zip -q "$OUT_DIR/$APP_NAME-dexed.apk" classes.dex); rm /tmp/classes.dex
...
if [ ! -f "$KEYSTORE" ]; then
    keytool -genkey -v -keystore "$KEYSTORE" -alias androiddebugkey -keyalg RSA \
        ...
        -dname "CN=CarForge Debug, OU=CarrPod, O=CSM, L=Citadel, ST=Citadel, C=US" 2>/dev/null
fi
...
cp "$OUT_DIR/$APP_NAME-v$VERSION_NAME.apk" "$PROJECT_DIR/../../../../CSMDropBox/$APP_NAME-v$VERSION_NAME.apk" 2>/dev/null && echo "  Copied to CSMDropBox" || true
cp "$OUT_DIR/$APP_NAME-v$VERSION_NAME.apk" "$PROJECT_DIR/../../../../$APP_NAME-v$VERSION_NAME.apk" 2>/dev/null && echo "  Copied to repo root" || true
exit 0
