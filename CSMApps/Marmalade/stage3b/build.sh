#!/bin/bash
# ============================================================
# build.sh — No-Gradle APK Build for Marmalade Stage 3b
...
echo "============================================================"
echo "  MARMLADE Stage 3b — No-Gradle Build"
echo "  Package: $PACKAGE  |  Version: $VERSION_NAME"
echo "============================================================"

if [ ! -f "$ANDROID_JAR" ]; then
    echo "ERROR: android.jar not found at $ANDROID_JAR"
    echo "Run 'bash setup-env.sh' first."
    exit 1
fi
...
# ─── STEP 1: aapt2 compile (resources → .flat) ──────────
...
# ─── STEP 2: aapt2 link (.flat → base APK + R.java) ────
...
# ─── STEP 3: javac (Java sources → .class) ──────────────
...
if [ "$CLASS_COUNT" -gt 0 ]; then
    # Collect all .class file directories for d8 input
    "$D8" \
        --lib "$ANDROID_JAR" \
        --min-api $MIN_SDK \
        --output "$OBJ_DIR" \
        ...
        2>&1 | tail -1 || {
            echo "  d8 failed, trying alternate method..."
    ...
    echo "  No classes to dex. Creating stub."
    "$D8" --lib "$ANDROID_JAR" --min-api $MIN_SDK --output "$OBJ_DIR" --no-desugaring /dev/null 2>/dev/null || true
fi
...
if [ ! -f "$KEYSTORE" ]; then
    keytool -genkey -v \
        ...
        -storepass android \
        -keypass android \
        -dname "CN=Marmalade Debug, OU=Aegis, O=CarrPod, L=Lab, ST=Dev, C=US" \
        2>/dev/null
...
# Copy to root
cp "$OUT_DIR/$APP_NAME-v$VERSION_NAME.apk" "$PROJECT_DIR/../../../../Marmalade-v$VERSION_NAME.apk" 2>/dev/null && \
    echo "  Copied to repo root: Marmalade-v$VERSION_NAME.apk" || true
